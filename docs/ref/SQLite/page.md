...
레코드(cell)마다 **2바이트 포인터가 정확히 하나씩** 있고 키 순서로 정렬되어 있다. 지난번 정리한 InnoDB의 sparse directory(4~8개 레코드당 슬롯 1개 + 연결리스트)와 정반대로, **PostgreSQL와 같은 dense 구조**다.
...
셀 컨텐츠 영역은 페이지 끝에서부터 역방향으로 자란다 - PostgreSQL의 `pd_upper`와 같은 패턴. 헤더+포인터 배열과 셀 컨텐츠 영역 사이의 빈 공간이 미할당(unallocated) 영역이다.
