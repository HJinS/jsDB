...
class FreeSpaceManager (
    ...
    fun getFreePageID(): Long{
        bufferPoolManager.fetchPage(META_PAGE_ID).use{handle ->
            handle.asWriteView { buffer ->
                ...
                if(freeListHeadPageID != INVALID_PAGE_ID){
                    val nextFreePageID = bufferPoolManager.fetchPage(freeListHeadPageID).use { freePageHandle ->
                        freePageHandle.asReadView { freeBuffer ->
    ...
    fun addFreePageID(newFreePageID: Long){
        bufferPoolManager.fetchPage(META_PAGE_ID).use{handle ->
            handle.asWriteView { buffer ->
                ...
                if(freeListHeadPageID != INVALID_PAGE_ID){
                    bufferPoolManager.fetchPage(newFreePageID).use { newFreePageHandle ->
                        newFreePageHandle.asWriteView { newFreeBuffer ->
