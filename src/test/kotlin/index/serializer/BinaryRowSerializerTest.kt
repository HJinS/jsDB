...
import index.util.RowColumnSchema
...
class BinaryRowSerializerTest: FunSpec({
    listOf(
        listOf<Number>(10, 5230L) to RowColumnSchema(
            listOf(
                RowColumn("count", ColumnType.INT),
                RowColumn("largeCount", ColumnType.LONG)
            )
        ),
        listOf(10L) to RowColumnSchema(
            listOf(
                RowColumn("id", ColumnType.LONG)
            )
        ),
        listOf("Test Code") to RowColumnSchema(
            listOf(
                RowColumn("name", ColumnType.STRING)
            )
        ),
        listOf(true) to RowColumnSchema(
            listOf(
                RowColumn("isActive", ColumnType.BOOLEAN)
            )
        ),
        listOf(1.toByte()) to RowColumnSchema(
            listOf(
                RowColumn("byte", ColumnType.BYTE)
            )
        ),
        listOf(10.toShort()) to RowColumnSchema(
            listOf(
                RowColumn("idShort", ColumnType.SHORT)
            )
        ),
        listOf(10.0f) to RowColumnSchema(
            listOf(
                RowColumn("price", ColumnType.FLOAT)
            )
        ),
        listOf(10.0) to RowColumnSchema(
            listOf(
                RowColumn("id", ColumnType.DOUBLE)
            )
        ),
        listOf(LocalDate.of(2025, 1, 1)) to RowColumnSchema(
            listOf(
                RowColumn("date", ColumnType.LOCAL_DATE)
            )
        ),
        listOf(LocalDateTime.of(2025, 1, 1, 12, 0, 0)) to RowColumnSchema(
            listOf(
                RowColumn("dateTime", ColumnType.LOCAL_DATE_TIME)
            )
        ),
        listOf(Instant.ofEpochSecond(100)) to RowColumnSchema(
            listOf(
                RowColumn("epoch", ColumnType.INSTANT)
            )
        ),
        listOf(UUID.randomUUID()) to RowColumnSchema(
            listOf(
                RowColumn("uuid", ColumnType.UUID)
            )
        ),
        listOf(ByteBuffer.allocate(2).putShort(10).array()) to RowColumnSchema(
            listOf(
                RowColumn("bytes", ColumnType.BYTES)
            )
        ),
        listOf(10, "Alice", LocalDate.of(2025, 5, 10)) to RowColumnSchema(
            listOf(
        ...
        listOf(true, 10.0f, UUID.randomUUID()) to RowColumnSchema(
            listOf(
                RowColumn("isActive", ColumnType.BOOLEAN),
                RowColumn("price", ColumnType.FLOAT),
                RowColumn("uuid", ColumnType.UUID)
        ...
        listOf(Instant.ofEpochSecond(100), "Alice", ByteBuffer.allocate(2).putShort(10).array()) to RowColumnSchema(
            listOf(
                RowColumn("epoch", ColumnType.INSTANT),
                RowColumn("name", ColumnType.STRING),
                RowColumn("bytes", ColumnType.BYTES)
